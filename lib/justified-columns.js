(function (root, factory) {
    var JustifiedColumns = factory(root);

    if ( typeof define === 'function' && define.amd ) {
        define([], function () {
            return JustifiedColumns;
        });
    } else if ( typeof exports === 'object' ) {
        module.exports = JustifiedColumns;
    } else {
        root.JustifiedColumns = JustifiedColumns;
    }
    ...
    function JustifiedColumns(config) {
        ...
        this.grid.JustifiedColumns = this;
    ...
    JustifiedColumns.prototype.setGrid = function(grid) {
    ...
    JustifiedColumns.prototype.justify = function() {
        var self = this,
                    ...
                    if (stretchable) {
                        ...
                        if (stretchable.nodeName !== 'IMG') {
                            ...
                            if (stretchable.parentNode.className !== WRAPPER_CLASS) {
                                ...
                                wrapper.style.fontSize = 0; //otherwise IE will add just a liiiiitle bit of extra space
        ...
        if (NEEDS_WRAPPER) {
            var resizeHeight = referenceColumn + 2,
                gridStyle = getComputedStyle(this.grid);
            if (gridStyle.boxSizing === 'border-box') {
                ['paddingTop', 'paddingBottom', 'borderTopWidth', 'borderBottomWidth'].forEach(function(spacer) {
                    resizeHeight += pxAsNum(gridStyle[spacer]);
                });
            }
            if (Math.abs(this.grid.offsetHeight - resizeHeight) >= 2) {
                this.grid.style.height = resizeHeight + 'px';
            }
        }

    };

    JustifiedColumns.prototype.reset = function() {
    ...
    JustifiedColumns.prototype.enable = function() {
        if (this.autoResize) {
    ...
    JustifiedColumns.prototype.disable = function() {
        if (this.autoResize) {
