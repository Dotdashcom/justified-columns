JustifiedColumns = (function(window) {
	...
	Justifier.prototype.justify = function() {
	    ...
	    var self = this,
	    	columns = [], 
	        i = 0, 
	        ii = window.getComputedStyle(this.grid).columnCount*1, 
	        currentColumn = -1,
	        tallestColumn = 0,
	        currentOffset;
	    
	    //set up the column data structure
	    for (; i < ii; i++) {
	        columns.push({
	            items: [],
	            height: 0
	    ...
	    [].slice.call(this.grid.children).forEach(function(gridItem) {
	        ...
	        if (gridItemOffsetLeft !== currentOffset) {
	            ...
	            columns[currentColumn].height += pxAsNum(gridItemStyle.getPropertyValue('margin-top'));
	        }
