JustifiedColumns = (function(window) {
    ...
    var DEFAULT_THROTTLE = 25,
        ...
        NEEDS_WRAPPER = true || document.documentElement.style.objectFit === 'undefined';
	...
	Justifier.prototype.justify = function() {
        ...
        console.log("referenceColumn", referenceColumn);
            ...
            if (column.height < referenceColumn || (self.shrink && column.height > referenceColumn)) {
	            ...
	            column.items.forEach(function(gridItem, itemIndex) {
                    ...
                    if (stretchable) {
                        ...
                        console.log("stretchableDelta", stretchableDelta);
                        var newHeight = gridItem.stretchableHeight + stretchableDelta;
                        console.log("gridItem.stretchableHeight", gridItem.stretchableHeight, 'newHeight', newHeight);
                        //if it's not an image, just set max/min-height and call it a day
                        if (stretchable.nodeName !== 'IMG') {
                            stretchable.style[(self.shrink ? 'max' : 'min') + 'Height'] = newHeight + 'px';
                        }
                            ...
                            if (stretchable.parentNode.className !== WRAPPER_CLASS) {
                                ...
                                wrapper.style[(self.shrink ? 'max' : 'min') + 'Height'] = newHeight + 'px';
                                stretchable.parentNode.replaceChild(wrapper, stretchable);
                            }
                            if (self.shrink) {
                                stretchable.style.marginTop = -0.25 * (newHeight - stretchable.offsetHeight) + 'px';
                                stretchable.style.marginBottom = 0.25 * (newHeight - stretchable.offsetHeight) + 'px';
                            }
                            else {
